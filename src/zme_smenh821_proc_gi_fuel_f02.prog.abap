...
*----------------------------------------------------------------------*
CONSTANTS: c_progname   TYPE zmm_flcm_parms-progname      VALUE 'SMENH821',
...
* Put valid status in a range.
  CLEAR: lwa_range.
  lwa_range-sign   = c_sign.
  lwa_range-option = c_option.
  lwa_range-low    = c_ready.
  APPEND lwa_range TO lr_status.

  lwa_range-low    = c_error.
  APPEND lwa_range TO lr_status.

* Retrieve data valid transaction types and movement types from zmm_flcm_parms
  SELECT * FROM zmm_flcm_parms
           INTO TABLE i_zmm_flcm_parms
           WHERE progname EQ c_progname
           AND zparm_nm EQ c_zparm_nm1.
  IF sy-subrc EQ 0.
    LOOP AT i_zmm_flcm_parms  INTO wa_zmm_flcm_parms.
      IF wa_zmm_flcm_parms-zparm_nm EQ c_zparm_nm1.
        lwa_range-sign   = wa_zmm_flcm_parms-zparm_sign.
        lwa_range-option = wa_zmm_flcm_parms-zparm_opt.
        lwa_range-low    = wa_zmm_flcm_parms-zval_from.
...
********************************************************************************************************************************
  DATA:
      lo_trx_proc_gi_fuel TYPE REF TO zcl_flcm_trx_proc_gi_fuel,
...
* Create instance of the class ZCL_FLCM_TDS_PROC_BOL
  CREATE OBJECT lo_trx_proc_gi_fuel
    EXPORTING
      it_trans     = i_zmm_flcm_tds.

* Execute the program logic in class ZCL_FLCM_TDS_PROC_BOL
  lo_trx_proc_gi_fuel->execute( ).
